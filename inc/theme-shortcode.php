...
function wymusic($atts, $content = null, $code = "")
{
    ...
    $return .= $content;
    $return .= '&auto=' . Lichu_option('g_163mic', false) . '&height=66"></iframe></div>';
    return $return;
}
...
{
    extract(shortcode_atts(array("title" => __('标题内容', 'Lichu')), $atts));
...
{
    extract(shortcode_atts(array("notice" => '<div class="alert alert-primary text-center" role="alert">' . __('温馨提示：此处内容已隐藏，<a href="#comments">回复</a>后刷新页面即可查看！', 'Lichu') . '</div>'), $atts));
    $userEmail = null;
