...
{
    return Lichu_option('g_excerpt_length', '260');
}
...
// 生成适合特色图的比例图片
add_image_size('Lichu-thumbnail', 512, 288, true);
...
function post_thumbnail()
{
    ...
    $img_url = wp_get_attachment_image_src($img_id, 'Lichu-thumbnail');
    if (is_array($img_url)) {
        $img_url = $img_url[0];
    }
    if (has_post_thumbnail()) {
        ...
        if (!empty($img_val)) {
            ...
            echo '<img src="' . Lichu_option('g_postthumbnail', ASSET_PATH . '/assets/img/default.jpg') . '" />';
        }
    }
}
...
function comment_scripts()
{
    ...
    wp_localize_script('comment', 'ajaxcomment', array(
        ...
        'compost' => __('评论正在提交中', 'Lichu'),
        'comsucc' => __('评论提交成功', 'Lichu'),
    ));
}
            ...
            <div class="info clearfix">
                ...
                <div class="meta clearfix">
                    ...
                    <div class="tool reply ml-2 d-inline-block float-right">
                        <?php
                        $defaults = array('add_below' => 'comment', 'respond_id' => 'respond', 'reply_text' => '<i class="kicon i-reply"></i><span class="ml-1">' . __('回复', 'Lichu') . '</span>');
...
if (!Lichu_option('g_gutenberg', false)) {
    // 禁用 Gutenberg 编辑器
...
function seo_meta_boxes()
{
    ...
    add_meta_box('meta-box-id', __('SEO 设置', 'Lichu'), 'post_seo_callback', $post_types);
}
...
if (Lichu_option('g_image_filter', true)) {
...
// 主页轮播
function Lichu_carousel()
{
    if (Lichu_option('g_carousel', false)) {
        $slide = 0;
        $item = 0;
        $output = '<div id="indexCarousel" class="carousel article-carousel slide" data-ride="carousel"> <ol class="carousel-indicators">';

        if (!empty(Lichu_option('carousel_group'))) {

            foreach (Lichu_option('carousel_group') as $group_item) {
                $active = null;
                if ($slide == 0) {
                    $active = 'class="active"';
                }
