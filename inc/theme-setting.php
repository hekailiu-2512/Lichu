...
function title($title, $sep)
{
    ...
    if ($paged >= 2 || $page >= 2) {
        $title = "{$title} {$sep} " . sprintf(__('第 %s 页', 'Lichu'), max($paged, $page));
    }
    return $title;
}
...
function keywords()
{
    ...
    $keywords = '';
    if (is_home()) {
        $keywords = Lichu_option('seo_keywords');
    } elseif (is_single()) {
...
function description()
{
    ...
    $description = '';
    if (is_home()) {
        $description = Lichu_option('seo_description');
    } elseif (is_single()) {
...
add_filter('robots_txt', function ($output, $public) {
    if ('0' == $public) {
        ...
        if (!empty(Lichu_option('seo_robots_fieldset')['seo_robots'])) {
            $output = esc_attr(strip_tags(Lichu_option('seo_robots_fieldset')['seo_robots']));
        }
        return $output;
    }
}, 10, 2);
...
function mourning()
{
    if (is_home() && Lichu_option('g_rip', false)) {
...
function share_thumbnail_url()
{
    ...
    if (has_post_thumbnail($post->ID)) {
        ...
        if (!empty($strResult[1])) {
            ...
            $img = Lichu_option('seo_shareimg', ASSET_PATH . '/assets/img/default.jpg');
        }
    }
    return $img;
}
