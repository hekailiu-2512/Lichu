...
// CDN 资源地址
if (Lichu_option('g_cdn', false)) {
    $asset_path = 'https://cdn.jsdelivr.net/gh/hekailiu-2512/Lichu@v' . THEME_VERSION;
} else {
...
function init_theme()
{
    ...
    if ('themes.php' == $pagenow && isset($_GET['activated'])) {
        wp_redirect(admin_url('admin.php?page=Lichu-options'));
        exit;
    }
}
...
{
    load_theme_textdomain('Lichu', get_template_directory() . '/languages');
}
...
function theme_autoload()
{
    if (!is_admin()) {
        ...
        if ((Lichu_option('g_article_lightgallery', true) && is_single()) || (Lichu_option('g_page_lightgallery', true) && is_page())) {
        ...
        if (Lichu_option('g_animate', false)) {
            wp_enqueue_style('animate', ASSET_PATH . '/assets/css/animate.min.css', array(), '4.1.1');
        }
        if (Lichu_option('g_fontawesome', false)) {
            wp_enqueue_style('fontawesome', ASSET_PATH . '/assets/css/fontawesome.min.css', array(), '5.15.2');
        }
        wp_enqueue_style('Lichu', ASSET_PATH . '/style.css', array(), THEME_VERSION);
        if (is_child_theme()) {
            wp_enqueue_style('Lichu-child', get_stylesheet_uri(), array(), wp_get_theme()->get('Version'));
        }
        if (Lichu_option('g_adminbar', true)) {
            $admin_bar_css = "
            ...
            if (current_user_can('level_10')) {
                wp_add_inline_style('Lichu', $admin_bar_css);
            }
        }
        wp_add_inline_style('Lichu', "
        @media screen and (min-width: 992px) {
            .k-nav .navbar-brand h1 {
                color: " . Lichu_option('g_nav', '#ffffff') . ";
            }
        ...
        ");
        if (Lichu_option('g_sticky', false)) {
            wp_add_inline_style('Lichu', '.sticky-sidebar {
                position: sticky;
                top: 8px;
                height: 100%;
            }');
        ...
        wp_enqueue_script('Lichu', ASSET_PATH . '/assets/js/Lichu.js', array('jquery'), THEME_VERSION, true);

        $data = array(
            'site' => home_url(),
            'directory' => ASSET_PATH,
            'alipay' => Lichu_option('g_donate_fieldset')['g_donate_alipay'] ?? '',
            'wechat' => Lichu_option('g_donate_fieldset')['g_donate_wechat'] ?? '',
            'repeat' => __('您已经赞过了', 'Lichu'),
            'thanks' => __('感谢您的支持', 'Lichu'),
            'donate' => __('打赏作者', 'Lichu'),
            'scan'   => __('扫码支付', 'Lichu'),
        );
        wp_localize_script('Lichu', 'Lichu', $data);
    }
}
add_action('wp_enqueue_scripts', 'theme_autoload');

// 后台资源加载
function Lichu_admin_enqueue()
{
...
add_action('admin_enqueue_scripts', 'Lichu_admin_enqueue', 20);
...
// 移除自动保存、修订版本
if (Lichu_option('g_post_revision', true)) {
...
// Gravatar 加速服务
if (Lichu_option('g_replace_gravatar_url_fieldset')['g_replace_gravatar_url'] ?? true) {
    function replace_gravatar_url($avatar)
    {
        $gravatar_server_list = array(
            'geekzu' => 'sdn.geekzu.org',
            'loli' => 'gravatar.loli.net',
            'other' => Lichu_option('g_replace_gravatar_url_fieldset')['g_custom_gravatar_server'] ?? null,
        );
        $gravatar_server = $gravatar_server_list[Lichu_option('g_replace_gravatar_url_fieldset')['g_select_gravatar_server'] ?? 'geekzu'];
...
$myUpdateChecker = PucFactory::buildUpdateChecker(
    'https://cdn.jsdelivr.net/gh/hekailiu-2512/Lichu/inc/update-checker/update.json',
    get_template_directory() . '/functions.php',
    'Lichu'
);

// 禁止生成多种尺寸图片
if (Lichu_option('g_removeimgsize', false)) {
...
function custom_upload_filter($file)
{
    ...
    $prdfix = Lichu_option('g_renameother_fieldset')['g_renameother_prdfix'] . '-';

    $img_mimes = array('jpg', 'JPG', 'jpeg', 'JPEG', 'gif', 'GIF', 'png', 'PNG', 'bmp', 'BMP', 'webp', 'WEBP', 'svg', 'SVG');

    $str = Lichu_option('g_renameother_fieldset')['g_renameother_mime'];
    $arr = explode("|", $str);
    ...
    if (Lichu_option('g_renameimg', false)) {
    ...
    if (Lichu_option('g_renameother_fieldset')['g_renameother']) {
...
// 仅搜索文章标题
if (Lichu_option('g_search', false)) {
